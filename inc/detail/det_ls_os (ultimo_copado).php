...
$table_stop=substr($fec2,0,4);
	...
	echo "<center><table class=myTable-gray width=95%>";
		echo"<tr>";
			echo "<td colspan=10 class=titulo>".$_POST["os"]." - ".month_toString(substr($fec2,5,2),1,1)."</td>";
		echo "</tr>";
	...
	for($i=$table_in;$i<=$table_stop;$i++){
		...
		$qr="SELECT op.pac_id, op.op_date,op.op_coseg,pac.pac_os, pac.pac_domicilio,pac.pac_dpto,pac.pac_telefono"
				.",sum( op.cant_horas) as cont,op.cant_horas,pac.pac_apellido,CONCAT(pac.pac_apellido,' ',pac.pac_nombre) as nombre,COUNT(op.pac_id) as cont2,"
			...
			echo "<tr valign=top>";
				...
				//if($row1["cant_horas"]==4){				
					...
					elseif($_SESSION["usr_role"]=="admin" && $_SESSION["usr_level"]>=5){	
						echo "<td align=center><b>".($row1["cont"]*90)."</b></td>";
					}
				//}
