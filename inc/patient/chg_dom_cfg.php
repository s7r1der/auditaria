...
//	echo "<CENTER><font size=6 color=white><STRONG>HISTORIA CLINICA</STRONG></font><br><br></CENTER>";
	...
	if($_SESSION["dato_busq"]!=""){
		...
		if(isset($_SESSION["sel_var"]) && $_SESSION["sel_var"]==1){
			$res=mysqli_query($conn,"select *,pac_id,CONCAT(pac_apellido,' ',pac_nombre) as nombre FROM paciente where ".$_SESSION["opt_busq"]. "='"
			.$_SESSION["dato_busq"]."' order by pac_id")or die(mysql_error($conn));
		}else{
			$res=mysqli_query($conn,"select *,pac_id,CONCAT(pac_apellido,' ',pac_nombre) as nombre FROM paciente where ".$_SESSION["opt_busq"]. " like'%"
				.$_SESSION["dato_busq"]."%' order by pac_id")or die(mysql_error($conn));
			$_SESSION["sel_var"]=0;
		}
	}else{
